...
>> In build we are using the MAVEN tool in order to build the entire source code .
>> After build we have to test the code by using some automations tools based on code which we have written based on programing language will use tools 
>> For ex python code we are using robotic framework 
>> and java we are using selenium , cucumber 
>> in devops for t automate the testcases we are using tool called "JENKINS " .
